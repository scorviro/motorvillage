...
export default function DesktopHomepage({ logoPaths, isVisible, preloadedImages, isLoaderComplete = true, loadProgressRef, isMobile = false }: HomepageProps) {
  ...
  const lastSuccessfulImageRef = useRef<HTMLImageElement | null>(null);
  ...
  useEffect(() => {
    ...
    const drawFrame = (index: number) => {
      ...
      if (img && img.naturalWidth > 0) {
        // Store as last successful image for fallback
        lastSuccessfulImageRef.current = img;
      ...
      } else if (lastSuccessfulImageRef.current) {
        // LAYER 2: "Last Drawn Image" fallback — NEVER show a black screen
        const fallback = lastSuccessfulImageRef.current;
        if (drawCacheRef.current) {
          const { drawWidth, drawHeight, offsetX, offsetY } = drawCacheRef.current;
          const canvasWidth = canvas.width / (lastDprRef.current || 1);
          const canvasHeight = canvas.height / (lastDprRef.current || 1);
          ctx.clearRect(0, 0, canvasWidth, canvasHeight);
    ...
    const handleScrollUpdate = (scrollY: number) => {
      ...
      // Mobile Sliding Window Prefetching
      if (isMobile) {
    ...
    const lenis = new Lenis({
      lerp: isMobile ? 0.05 : 0.07,
      orientation: "vertical",
      gestureOrientation: "vertical",
      smoothWheel: true,
      wheelMultiplier: 0.9,
      touchMultiplier: isMobile ? 0.4 : 1.2, // Slow down mobile touch scroll 3x
      autoRaf: false,
      syncTouch: false,
    });
